...
def submit_solution():
    ...
    # Test the solution
    passed = test_solution(code, problem['test_cases'], language)
...
def test_solution(code, test_cases, language):
    try:
        for test_case in test_cases:
            if language == 'python':
                # Create temp file and test
                with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
                    f.write(code + f"\nprint(Solution().twoSum({test_case['input']}))")
                    temp_file = f.name
                ...
                # Simple output comparison (in real app, use proper parsing)
                if test_case['output'] not in result.stdout:
                    return False
        return True
    except:
        return False
